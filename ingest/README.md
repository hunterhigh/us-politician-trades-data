...
`current.json` 是已部署生产入口。当前处于 `bootstrap_empty`：只发布来源健康与覆盖状态，自动资格校验和完整候选快照尚未接入该入口。

当第一批记录通过来源、身份、时效、修订、证据和自动资格校验后，用完整候选输出替换对应规范数组；隔离记录不能进入规范财务数组。普通生产发布不得使用 `--allow-empty-production`。该开关只允许所有人物、交易和持仓均为空的显式启动版本，不能绕过非空数据的校验。
