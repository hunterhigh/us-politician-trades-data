...
MARKET_SOURCE_IDS = {MARKET_SOURCE_ID, TWELVE_DATA_SOURCE_ID}
SOURCE_HEALTH_IDS = DISCLOSURE_SOURCE_IDS | GUIDANCE_SOURCE_IDS | MARKET_SOURCE_IDS
...
SOURCE_ALIASES = {
    ...
    "twelve data split-adjusted eod": TWELVE_DATA_SOURCE_ID,
}
...
def prepare_market(rows: list[dict[str, Any]], *, demo: bool) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    for row in rows:
        ...
        expected_feed = "sip" if row["source_id"] == MARKET_SOURCE_ID else "twelve_data"
        if not demo and (row.get("feed") != expected_feed or row.get("timeframe") != "1Day"):
            raise ProcessingError(f"Market row {ticker} has an invalid source feed or timeframe")
    ...
    for row in rows:
        ...
        if not snapshot or not snapshot.get("price_history"):
            if demo and snapshot:
                ...
                row.setdefault("price_source_id", snapshot["source_id"])
                continue
        ...
        row["performance_basis"] = (
            ...
            "split_adjusted_twelve_data_eod_close_on_or_after_event_date")
        row["performance_as_of_date"] = snapshot["as_of_date"]
        row["price_source_id"] = snapshot["source_id"]
...
def build_market_moves(rows: list[dict[str, Any]], market: dict[str, dict[str, Any]], cutoff: date, days: int) -> list[dict[str, Any]]:
    ...
    for ticker, items in grouped.items():
        ...
        result.append({
            ...
            "price_source_id": snapshot["source_id"] if snapshot else None,
        })
...
def build_snapshot(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    meta["market_price_policy"] = "source_labeled_split_adjusted_completed_eod"
    ...
    result.update({
        ...
        "processing": {
            ...
            "market_source_ids": sorted({row["source_id"] for row in market_rows}),
