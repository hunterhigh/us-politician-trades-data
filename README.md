...
House与Senate来源候选通过显式共同截止模式合并。由于两源申报时间目前都只有日期精度，统一候选使用最早来源运行日之前的最后完整UTC日，过滤该水位之后的申报，并裁剪无事实引用的人物；来源相差超过两天、源候选自身越过原生截止时间或行情数据缺少独立截止策略时失败关闭。当前GitHub生产候选合并为116人、4,502笔交易，截止到2026-09-19，未额外裁掉交易；每源原生截止时间、输入哈希和过滤前后数量写入独立机器审计。
